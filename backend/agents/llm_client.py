...
def _normalize_base_url(url: str) -> str:
    """Tolerate a base URL that already includes the endpoint or a trailing
    slash, so both '.../v1' and '.../v1/chat/completions' resolve correctly.
    (A common footgun: pasting the full endpoint into OPENROUTER_BASE_URL, which
...
def _gateway_headers() -> dict:
    """Common headers for the OpenAI-compatible gateway. The browser-style
    User-Agent matters: some gateways sit behind Cloudflare, whose Browser
    Integrity Check rejects urllib's default UA with 'error code: 1010'."""
    headers = {
...
def _openrouter_chat(model: str, messages: list[dict], max_tokens: int,
    ...
    url = f"{_normalize_base_url(SETTINGS.openrouter_base_url)}/chat/completions"
    try:
...
def list_models(timeout: int | None = None) -> dict:
    ...
    headers = _gateway_headers()
    url = f"{_normalize_base_url(SETTINGS.openrouter_base_url)}/models"
    try:
